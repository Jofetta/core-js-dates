...
function formatDate(date) {
  const timeUTC = new Date(date).getUTCHours();
  const dayUTC = new Date(date).getUTCDate();
  const dateUTC = new Date(
    new Date(new Date(date).setDate(dayUTC)).setHours(timeUTC)
  );
...
function getCountWeekendsInMonth(month, year) {
  const lastDayOfMonth = getCountDaysInMonth(month, year);
  let weekendCount = 0;
...
function getWeekNumberByDate(date) {
  ...
  let weeks = Math.ceil((date - jan1) / (1000 * 60 * 60 * 24 * 7));
  if (jan1.getDay() === 0) {
    weeks += 1;
  }
  if (jan1.getDay() >= date.getDay() && date.getDay() !== 0) {
    weeks += 1;
  }
  return weeks;
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  const startDate = new Date(period.start);
  const finalDate = period.end;
  const changedFormatDate = finalDate.split('-');
  const formatted = [
    changedFormatDate[1],
    changedFormatDate[0],
    changedFormatDate[2],
  ...
  const schedule = [];
  let workDaysCounter = 1;
  let daysOffCounter = 1;
  while (startDate <= endDate) {
    if (workDaysCounter < countWorkDays && workDaysCounter !== 0) {
      schedule.push(
        `${startDate.getDate().toString().padStart(2, '0')}-${(Number(startDate.getMonth()) + 1).toString().padStart(2, '0')}-${startDate.getFullYear()}`
      );
      workDaysCounter += 1;
    } else if (workDaysCounter === countWorkDays && workDaysCounter !== 0) {
      schedule.push(
        `${startDate.getDate().toString().padStart(2, '0')}-${(Number(startDate.getMonth()) + 1).toString().padStart(2, '0')}-${startDate.getFullYear()}`
      );
      workDaysCounter = 0;
    } else if (workDaysCounter === 0 && daysOffCounter < countOffDays) {
      daysOffCounter += 1;
    } else {
      daysOffCounter = 1;
      workDaysCounter = 1;
